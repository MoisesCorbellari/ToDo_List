...
from project_todo_list.models.todo_list_client_model import ToDoListClient
...
def get_all_todo_list_client(db: Session = Depends(get_db)) -> List[ToDoListClientResponse]:
    return db.query(ToDoListClient).all()

@router.get("/{id_task_client}", response_model=ToDoListClientResponse)
def get_all_todo_list_client_by_id(id_task_client: int,
                                   db: Session = Depends(get_db)) -> List[ToDoListClientResponse]:
    return find_todo_list_client_by_id(id_task_client, db)

@router.post("",response_model=ToDoListClientResponse, status_code=201)
def create_todo_list_client_by_id(task_client_request: ToDoListClientRequest,
...
def update_todo_list_client_by_id(id_task_client: int,
                                  task_client_request: ToDoListClientRequest,
                                  db: Session = Depends(get_db)) -> ToDoListClientResponse:
    todo_list_client = find_todo_list_client_by_id(id_task_client, db)
...
def delete_todo_list_client_by_id(id_task_client: int,
...
def find_todo_list_client_by_id(id_task_client: int, db: Session) -> ToDoListClient:
